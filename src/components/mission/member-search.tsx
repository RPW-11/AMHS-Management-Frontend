...
const MemberSearch = ({
    ...
    return (
        <div className="relative">
            ...
            {searchedName && (
                <div className="absolute z-10 top-12 flex items-center justify-center w-full p-2 rounded-md border bg-white shadow">
                    {isFetching ? (
                        <LoadingSpinner size="sm" />
                    ) : fetchError ? (
                        <h3 className="text-sm text-destructive">
                            {fetchError}
                        </h3>
                    ) : employees.length === 0 ? (
                        ...
                        <div className="space-y-2 w-full">
                            {employees.map((emp) => (
                                <div
                                    onClick={() =>
                                        handleAddMemberToMission(emp.id)
                                    }
                                    key={emp.id}
                                    className="flex justify-start hover:bg-gray-100 cursor-pointer rounded-md"
                                >
                                    <MissionMember {...emp} />
                                </div>
                            ))}
