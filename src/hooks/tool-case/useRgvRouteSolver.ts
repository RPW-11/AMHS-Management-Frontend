...
export const useRgvRouteSolver = () => {
    const submitRgvRoutePlan = useCallback(async (createRgvPlanReq: CreateRgvPathPlanReq): Promise<ApiError|null> => {
        try {
            ...
            const result = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_HOST}/missions/route-planning`, {
                method: "POST",
                body: requestForm
            })
